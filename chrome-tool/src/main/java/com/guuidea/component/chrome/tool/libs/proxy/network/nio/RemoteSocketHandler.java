...
public class RemoteSocketHandler extends SocketHandlerBase {
    private Logger logger = LoggerFactory.getLogger(RemoteSocketHandler.class.getName());

    public RemoteSocketHandler(ProxyAccount proxyAccount) throws IOException, InvalidAlgorithmParameterException {
        super(proxyAccount);
    }
        ...
        switch (request.type) {
            case ChangeRequest.CHANGE_SOCKET_OP:
                ...
                if ((key != null) && key.isValid()) {
                    ...
                    logger.warn("RemoteSocketHandler::processPendingRequest (drop): {}", ("" + key + request.socket));
                }
                break;
    ...
    public PipeWorker createPipe(ISocketHandler localHandler, SocketChannel localChannel, String ipAddress, int port) throws IOException {
        ...
        PipeWorker pipe = new PipeWorker(localHandler, localChannel, this, socketChannel, proxyAccount);
    ...
    private void write(SelectionKey key) throws IOException {
        ...
        if (queue != null) {
            ...
            logger.warn("RemoteSocket::write queue = null: {}", socketChannel);
            return;
        }
    }
